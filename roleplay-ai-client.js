...
function historyOf(cfg){return(cfg.getTrace?.()||[]).filter(x=>x?.ai).slice(-5).map(x=>({usuario:String(x.choice||"").slice(0,900),contraparte:String(x.reaction||"").slice(0,900)}))}
...
async function submit(cfg,state){
 ...
 try{
  ...
  cfg.pushTrace({choice:answer,points:p,feedback,ai:true,evaluation:ev,reaction:ev.reaccion,model:d.model});
  ...
  state.prompt=ev.reaccion||"Necesito que concretemos qué haría ahora y cómo verificaremos el resultado.";
