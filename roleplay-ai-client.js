...
function attach(cfg){
 const root=document.getElementById(cfg.root),guided=document.getElementById(cfg.guidedButton);if(!root||!guided||root.querySelector(".ai-pilot-callout"))return;const aiOnly=Boolean(cfg.actor?.aiOnly);
 guided.classList.remove("primary");guided.classList.add("secondary");guided.textContent=cfg.guidedLabel||"Opciones guiadas";if(aiOnly)guided.hidden=true;
 const box=document.createElement("div");box.className="ai-pilot-callout";box.innerHTML=`<b>${aiOnly?"Caso diseñado para IA":"Simulación con IA"}</b><span>${aiOnly?"Este escenario se practica con respuesta libre.":"Responde con tus propias palabras. La contraparte reaccionará a lo que escribas y una rúbrica específica de esta competencia evaluará tu trayectoria."}</span>`;
 ...
 const ai=document.createElement("button");ai.type="button";ai.className="primary";ai.textContent=aiOnly?"Iniciar conversación con IA →":"Responder libremente con IA →";buttons.appendChild(ai);
...
function render(cfg,state,feed){
 ...
 else{const g=document.getElementById("rpAiGuided");if(g)g.onclick=()=>cfg.guided();document.getElementById("rpAiSend").onclick=()=>submit(cfg,state);document.getElementById("rpAiAnswer")?.focus()}
}
