(()=>{"use strict";
const app=document.getElementById("app"),modalSelector="#expertModal,#paymentModal,#certificateModal,#adminPreviewModal:not([hidden])";let current="",fromHistory=false,scheduled=false,modalEntry=false,lastModal=null;
...
function closeModal(){document.querySelector("#expertClose,[data-payment-close],[data-certificate-close],#adminPreviewClose")?.click()}
function modalState(){const modal=document.querySelector(modalSelector),open=Boolean(modal);document.body.classList.toggle("desarrolla-modal-open",open);if(open&&!lastModal&&visible()&&!fromHistory){history.pushState({desarrolla:true,view:active(),modal:true},"",url(active()));modalEntry=true}if(!open&&lastModal&&modalEntry&&history.state?.modal){modalEntry=false;history.back()}lastModal=modal}
...
addEventListener("popstate",event=>{if(!visible())return;if(document.querySelector(modalSelector)){modalEntry=false;fromHistory=true;closeModal();setTimeout(()=>{fromHistory=false},0);return}restore(event.state?.view||location.hash.slice(1)||"home")});
