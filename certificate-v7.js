(() => {
  ...
  async function downloadPdf(data, route, button) {
    ...
    try {
      ...
      doc.setFont('helvetica', 'normal'); doc.setFontSize(6.5); doc.setTextColor(0, 123, 133); doc.textWithLink('Verificar autenticidad en desarrolla.movidasst.com', 27, 183, { url: `https://desarrolla.movidasst.com/verificar.html?codigo=${encodeURIComponent(data.codigo)}` });
      ...
      doc.setTextColor(100, 116, 139); doc.setFontSize(7); doc.text(data.codigo, 25, 187); doc.text('La Academia Movida de SST · www.movidasst.com', 272, 187, { align: 'right' });
      doc.setTextColor(0, 123, 133); doc.setFontSize(6.5); doc.textWithLink('Verificar certificado', 25, 192, { url: `https://desarrolla.movidasst.com/verificar.html?codigo=${encodeURIComponent(data.codigo)}` });
      if (data.prueba) watermark(doc);
